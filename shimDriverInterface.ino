...
volatile bool early_timer_in_flight = false;
volatile bool late_timer_in_flight = false;



const int ctrlBuffer_length = 1000;
...
void setDACVal() {
  ...
  if(early_timer_in_flight){
    Serial.println("early by:");
    Serial.println(micros()-timer);
              Serial.send_now();

    early_timer_in_flight = false;
  }
//  } else if(!late_timer_in_flight && blkIdx ==0 && repIdx ==0) {
//    timer = micros();
//    late_timer_in_flight = true;
//    Serial.println("starting late timer");
//  } else if(late_timer_in_flight && blkIdx ==0 && repIdx ==0) {
//    Serial.println("~~~~~~~~~Resetting Late timer: no update~~~~~~~~~~");
//    timer = micros();
//    late_timer_in_flight = true;
//  }
...
void loop() {
  ...
  if (!Serial.dtr()) {
    // if serial device disconnects, zero all outputs and wait for a new connection
    zero_all();
    delay(500);
  }
  switch (mode) {
    ...
    case MODE_HEADER:
      if (Serial.available()) {
        ...
        int ctrlBuff_readlen =
          Serial.readBytesUntil('#', ctrlBuffer, ctrlBuffer_length);
...
//        Serial.println(Serial.available(), DEC);
    ...
    case MODE_BODY:
      if (read_float_dump()) {
        ...
        int repIdx;
        int blkIdx = computeBlockIdx(counter_tmp);
        if (blkIdx == -1) {
          blkIdx = 0; repIdx = 0;
          counter_tmp = 0;
        } else {
          repIdx = computeRepIdx(counter_tmp, blkIdx);
        }

        if(!early_timer_in_flight && !late_timer_in_flight && (blkIdx == 0) && (repIdx == 0)){
          timer = micros();
          early_timer_in_flight = true;
          Serial.println("started timer");
        } 
//        else if (late_timer_in_flight){
//          Serial.println("late by:");
//          Serial.println(micros()-timer);
//          late_timer_in_flight = false;
//        }
        Serial.send_now();
